...
$$
... where $N$ is the set of nodes, $|N|$ is the number of nodes in the graph and $d(i, j)$ is the distance (i.e., the length of the shortest path) between nodes $i$ and $j$.
...
```{r}
g <- graph_from_literal(A --+ X, X --+ Y, Y --+ Z, Z --+ B)
...
We define $d(i, j)$ if $j$ is unreachable from $i$ as $d(i, j) = 0$.
TODO: figure out whether 0 or infinity (contradictory definitions online; infinity makes more sense?)
